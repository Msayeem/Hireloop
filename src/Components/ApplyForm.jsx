...
import { postApplication } from '@/app/lib/jobs';
...
const ApplyForm = ({ job, user }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const submissionData = {
            jobId: job?._id,
            jobTitle: job?.jobTitle || job?.title, // Safe fallback
        ...
        try {
            ...
            alert('Failed to submit application. Server error.');
        } finally {
            setIsSubmitting(false);
        }
    }; // <-- This closing brace was missing or misplaced in your original code
        ...
        <div className="max-w-xl mx-auto p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="mb-6">
                ...
                <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50 mt-1">
                    Apply for {job?.title || job?.jobTitle || 'this position'}
                </h2>
                {user?.name && (
